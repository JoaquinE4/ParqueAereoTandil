...
export default function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 h-20 flex items-center ${scrolled ? 'bg-white shadow-md' : 'bg-transparent text-white'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          <a href="#inicio" className={`text-2xl font-heading font-bold ${scrolled ? 'text-brand-black' : 'text-white'}`}>
            PARQUE <span className="text-brand-orange">AÉREO</span>
          </a>

          {/* Menú Desktop con Divisores Verticales */}
          <ul className="hidden lg:flex items-center">
            {menuItems.map((item, index) => (
              <React.Fragment key={item.name}>
                <li>
                  <a href={item.href} className={`px-4 font-semibold hover:text-brand-orange transition-colors ${scrolled ? 'text-brand-black' : 'text-white'}`}>
                    {item.name}
                  </a>
                </li>
      ...
      {/* 1. INICIO (HERO) */}
      <header id="inicio" className="relative h-screen flex items-center justify-center text-center text-white bg-cover bg-center" style={{ backgroundImage: "linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('https://images.unsplash.com/photo-1510146753228-4f615b7a1002?auto=format&fit=crop&w=1920&q=80')" }}>
        <div className="px-4" data-aos="zoom-out">
          <h1 className="text-5xl md:text-7xl font-heading font-bold mb-6">Aventura en Tandil</h1>
          <p className="text-xl md:text-2xl mb-8 max-w-2xl mx-auto">Vuela sobre las sierras en el parque de aventura más emocionante de la región.</p>
          <a href="#contacto" className="bg-brand-orange text-white font-bold py-4 px-10 rounded-full hover:bg-brand-dark transition-all shadow-lg inline-block">Planear Visita</a>
        </div>
      </header>
